...
class BookHomeViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UNUserNotificationCenterDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate
{
    ...
    override func viewDidLoad() {
        ...
        emptyLbl.text = "You Have no Bookings Available".localized()
        
        emptyLbl.font = FontBook.Regular.of(size: 17)
        ...
        refreshControl.addTarget(self, action:  #selector(BookHomeViewController.refresh(sender:)), for: .valueChanged)
        tableView.addSubview(refreshControl)
        getMyBookings()
        
        NotificationCenter.default.addObserver(self, selector: #selector(BookHomeViewController.refresh(sender:)), name: NSNotification.Name(rawValue: "Refresh"), object: nil)
...
//        let url = "\(Constants.baseURL)/calenderbookingdetails"
        let url = APIList().getUrlString(url: .CALENDER_BOOKING_DETAILS)
        print(params)
        print(url)
        ...
        Alamofire.request(url,method: .post, parameters:params, headers:headers).responseJSON { response in
            ...
            if(response.result.isSuccess)
            {
                ...
                if let json = response.result.value {
                    print("Calender BOOKINGS JSON: \(json)") // serialized json response
                    let jsonResponse = JSON(json)
                    print(jsonResponse)
