...
//        IQKeyboardManager.sharedManager().enable = true
    ...
    func getChatsList()
    {
        ...
        if let accesstoken = UserDefaults.standard.string(forKey: "provider_id")
        {
            ...
            {
                senderId = UserDefaults.standard.object(forKey: "provider_id") as! String
            }
            
            var receiver = ""
            if SharedObject().hasData(value: receiverId){
                receiver = receiverId!
            }
            ...
            print("getChatsList Params = ",params)
            //            let url = "\(Constants.baseURL)chatlist"
            
            let url = ("\(Constants.SocketMessage)/userchatlist")
...
//                                let book = jsonResponse["booking_id"].stringValue
//                                if book != ""
//                                {
//                                    //SharedObject().hasData(value: book){
//                                    self.bookingId = book
//                                }
                                ...
                                self.userPicture.sd_setImage(with: URL(string: self.receiverProImage), placeholderImage: UIImage(named: "profile-1"))
                                self.sendProImage.sd_setImage(with: URL(string: self.senderProImage), placeholderImage: UIImage(named: "profile-1"))
    ...
    @IBAction func backPressed(_ sender: Any)
    {
        ...
        //                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "dismissBack"), object: nil)
        //            })
        //        })
        //
        self.dismiss(animated: true, completion: nil)
    ...
    @objc func messageReceived(_ notification: NSNotification)
    {
        if let data = notification.userInfo?["data"] as? [Any]
        {
            ...
            message["booking_id"].stringValue = response[0]["booking_id"].stringValue
    ...
    @IBAction func sendMessage(_ sender: Any)
    {
        ...
        if(appDelegate.socket != nil)
        {
            ...
            let content = self.growingTextView.textView.text!
            ...
            if(content.count > 0)
            {
                ...
                /*                let sname = ""//UserDefaults.standard.string(forKey: username)*/
                
                
                appDelegate.socket.emit("sendmessagefromprovider", ["sender_id":senderId,"reciever_id":self.receiverId!,"message_id":messageId,"content_type":"text","content":content,"Time":currentTime,"booking_id":booking_id])
        ...
        {
            self.growingTextView.textView.resignFirstResponder()
//            self.view.makeToast("Socket is not connected".localized())
...
//            appDelegate.socket.connect()
    ...
    func sendMessage(content :String,contentType :String,fileName :String = "")
    {
        ...
        if(appDelegate.socket != nil)
        {
            if(content.count > 0)
            {
                ...
                appDelegate.socket.emit("sendmessagefromprovider", ["sender_id":senderId,"reciever_id":self.receiverId!,"message_id":messageId,"content_type":contentType,"content":content,"Time":currentTime,"booking_id":booking_Id,"fileName":fileName])
    ...
    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentAt url: URL)
    {
        ...
        //        let setName = "FILE_" + getFileName() + String(describing: filename)
    ...
    {
        //        let itemUrl = URL(string: "http://iiswc.org/iiswc2012/sample.doc")
...
extension ChatViewController : SenderPDFTableViewCellDelegate
{
    func senderPDFDidTap(_ sender: SenderPDFTableViewCell, index: IndexPath)
    {
        ...
        print(" SenderPDFTableViewCellDelegate = ",self.messages[index.row])
...
extension ChatViewController : ReceiverPDFTableViewCellDelegate
{
    ...
    {
        print(" ReceiverPDFTableViewCellDelegate = ",self.messages[index.row])
